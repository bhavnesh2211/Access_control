module.exports = ( resources, knex, jwt ) => {
    ...
    resources.post ( "/newresource" , ( req, res ) => {
        ...
        if (cookie == undefined) {
            ...
            let user_id = jwt.verify ( token , "bhavnesh" ).user_id;
            console.log ( user_id )
    ...
    resources.put ( "/updateresource" , ( req, res ) => {
        ...
        if (cookie == undefined) {
            ...
            knex ( "Users" ).select ( "Role" ).where ( "Id", user_id )
            .then (( data ) => {
                if ( data[0].Role == "Superadmin" || data[0].Role == "editor") {
                    if ( req.body.Name == undefined || req.body.Type == undefined ) {
                        res.send ( "Please input valid data..." );
                    }else {
                        knex ( "Resources" ).update (req.body)
                        .then (( data ) => {
                            res.send ( "You have successfully add this resource....");
                            console.log ( data );
                        }).catch (( err ) => {
                    ...
                    res.send ( "Only super admin or editor can update resources....." );
                }
            }).catch (( err ) => {
